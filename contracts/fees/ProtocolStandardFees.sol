...
contract ProtocolStandardFees is IProtocolStandardFees, AccessManagedUUPSUpgradeable {
        ...
        returns (uint256)
    {
        ...
        uint256 feeRate;
        if (priceConfig.priceType == IAsset.PriceType.FIXED_PRICE) {
            feeRate = $.cryptoFeeRate;
        } else if (priceConfig.priceType == IAsset.PriceType.FIXED_FIAT_PRICE) {
            feeRate = $.fiatFeeRate;
        } else {
            feeRate = 0;
        }
        ...
        return (totalAmount * feeRate) / $.feeDenominator;
    }
