...
contract FiatSettlementCondition is ReentrancyGuardUpgradeable, TemplateCondition, IFiatSettlement {
  ...
  function fulfill(
    ...
    if (
      !$.nvmConfig.hasRole(_senderAddress, FIAT_SETTLEMENT_ROLE) || plan.owner == _senderAddress
    ) {
  ...
  function _getFiatSettlementConditionStorage()
    internal
    pure
    returns (FiatSettlementConditionStorage storage $)
  {
