...
contract NFT1155ExpirableCredits is NFT1155Base, OwnableUpgradeable {
  ...
  function burn(address _from, uint256 _id, uint256 _value) public virtual override {
    ...
    for (uint256 index = 0; index < _numEntries; index++) {
      ...
      if (
        entry.expirationSecs == 0 || block.timestamp < (entry.mintTimestamp + entry.expirationSecs)
      ) {
        if (_pendingToBurn <= entry.amountMinted) {
          _credits[_key].push(
            MintedCredits(_pendingToBurn, entry.expirationSecs, block.timestamp, false)
          );
          break;
        } else {
    ...
    for (uint256 index = 0; index < _length; index++) {
      if (
        ...
        (_credits[_key][index].expirationSecs == 0 ||
          block.timestamp <
          (_credits[_key][index].mintTimestamp + _credits[_key][index].expirationSecs))
      ) {
